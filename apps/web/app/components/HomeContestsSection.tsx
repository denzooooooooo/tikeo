...
const contests = [
  { id: '1', title: 'Concours Chanson 2024', participants: 1250, daysLeft: 5, image: 'https://images.unsplash.com/photo-1598488035139-bdbb2231ce04?w=600&q=80' },
  { id: '2', title: 'Votez pour le Meilleur Artiste', participants: 3400, daysLeft: 12, image: 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600&q=80' },
  { id: '3', title: 'Concours Photo Nature', participants: 890, daysLeft: 20, image: 'https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=600&q=80' },
];

export default function HomeContestsSection() {
  return (
            ...
            <Link key={contest.id} href={`/votes/${contest.id}`} className="group">
              <div className="relative rounded-3xl overflow-hidden transition-all duration-500 group-hover:scale-[1.02] card-shine" style={{ background: 'linear-gradient(145deg, rgba(255,255,255,0.07) 0%, rgba(255,255,255,0.02) 100%)', border: '1px solid rgba(255,255,255,0.1)' }}>
                <div className="relative h-52">
                  <Image src={contest.image} alt={contest.title} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                  ...
                  <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
                    ...
                    <div className="px-3 py-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, #5B7CFF, #7B61FF)' }}>
                      <span className="text-xs font-bold text-white">{contest.daysLeft}j restants</span>
                    </div>
                  </div>
                ...
                <div className="p-6">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between text-sm mb-2">
                      ...
                      <span className="text-white font-semibold">{contest.participants.toLocaleString()}</span>
                    </div>
                    <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${Math.min(100, (contest.participants / 4000) * 100)}%`, background: 'linear-gradient(90deg, #5B7CFF, #7B61FF, #A8D4FF)', boxShadow: '0 0 10px rgba(91,124,255,0.5)' }} />
                    </div>
                  </div>
                  <div className="flex items-center justify-between pt-4 border-t border-white/10">
                    <span className="text-sm text-white/50">{contest.participants.toLocaleString()} participants</span>
